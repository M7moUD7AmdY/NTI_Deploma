...
void UART_voidInit(void)
{
	...
	CLEAR_BIT(Local_u8uCSRCVal , UCSRC_UMSEL);
	/*disable parity bit*/
	CLEAR_BIT(Local_u8uCSRCVal,UCSRC_UPM1);
	CLEAR_BIT(Local_u8uCSRCVal,UCSRC_UPM0);
	/*Choose one stop bit */
	CLEAR_BIT(Local_u8uCSRCVal , UCSRC_USBS);
	/*clear clock polarity */
	CLEAR_BIT(Local_u8uCSRCVal , UCSRC_UCPOL);
